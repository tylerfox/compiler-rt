...
#define DEBUG_TYPE "splafe-stack"
#define __SAFESTACK_DEFAULT_STACK_SIZE          0x2000000

#define __SPLAFESTACK_TOTAL_STACK_SIZE          0x4000000
#define __SPLAFESTACK_PAGE_SIZE                 4096
#define __SPLAFESTACK_NUM_PAGES                 (__SAFESTACK_DEFAULT_STACK_SIZE / __SPLAFESTACK_PAGE_SIZE)
#define __SPLAFESTACK_TOTAL_NUM_PAGES           (__SPLAFESTACK_TOTAL_STACK_SIZE / __SPLAFESTACK_PAGE_SIZE)
#define MAX_PAGES_OFFSET                        5 
...
# define __SPLAFESTACK_UNSAFE_STACK_SIZE_OFFSET  0x290
...
#endif
__thread int splafestack_layout[__SPLAFESTACK_NUM_PAGES];

static void* getSplafeStackPointer(void *start, int offset) {
  int i = 0;
  while (offset > __SPLAFESTACK_PAGE_SIZE || !splafestack_layout[i]) {
    if (splafestack_layout[i]) {
      offset -= __SPLAFESTACK_PAGE_SIZE;
    }
    i++;
  }
  return start + ((i * __SPLAFESTACK_NUM_PAGES) + offset);
}
...
static inline void unsafe_stack_setup(void *start, size_t size) {  
  void* stack_ptr = getSplafeStackPointer(start, size) - 2*sizeof(struct tinfo);
...
static inline void layout_splafestack(void) {
  int withinBounds;
  int segments;
  do {    
    segments = 0;
    ...
    int padding_size = rand() % MAX_PAGES_OFFSET+1;
    for (int i = 0; i < padding_size; i++) {
      splafestack_layout[i] = 0;
    }    
    int index = padding_size;
    while (segments < __SPLAFESTACK_NUM_PAGES ) {
      int r = rand() % 2;
      splafestack_layout[index] = r;
      if (r) {
        segments++;
      }     
      index++;
      if (index > __SPLAFESTACK_TOTAL_NUM_PAGES) {
        withinBounds = 0;
        break;
      }
    }
    for (int i = index; i < __SPLAFESTACK_TOTAL_STACK_SIZE; i++) {
      splafestack_layout[i] = 0;
    }
  } while (!withinBounds);
}
...
void __llvm__safestack_init() {  
  ...
  //size_t size = __SAFESTACK_DEFAULT_STACK_SIZE;
  size_t size = __SPLAFESTACK_TOTAL_STACK_SIZE;// + 2*sizeof(struct tinfo);
